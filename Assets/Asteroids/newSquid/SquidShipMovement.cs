...
public class SquidShipMovement : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		...
		var dx = Input.GetAxis("Move Horizontal");
		var dy = Input.GetAxis("Move Vertical");
		...
		cam.fieldOfView += (fovTarg-cam.fieldOfView)*fovDrag/1500;
	}
	...
	void Update()
	{
		...
		var dx = Input.GetAxis("Look Horizontal");
		var dy = Input.GetAxis("Look Vertical");
		var dz = Input.GetAxis("Look Roll");
		dx *= dx*Mathf.Sign(dx);
		dy *= dy*Mathf.Sign(dy);
		dz *= dz*Mathf.Sign(dz);
